...
%token FUNCTION PRINT PRINTN UNSET RETURN ACCEPT REJECT QUITBIRD INT BOOL IP PREFIX PAIR QUAD EC SET STRING BGPMASK BGPPATH CLIST ECLIST IF THEN ELSE CASE TRUE FALSE RT RO UNKNOWN GENERIC GW NET MASK PROTO SOURCE SCOPE CAST DEST LEN DEFINED ADD DELETE CONTAINS RESET PREPEND FIRST LAST MATCH EMPTY WHERE EVAL
